...
import (
	...
	mieruinbound "github.com/InazumaV/V2bX/core/sing/mieru"
...
type mieruNodeState struct {
	info   *panel.NodeInfo
	config *conf.Options
	users  map[string]panel.UserInfo
}

func getInboundOptions(tag string, info *panel.NodeInfo, c *conf.Options, users []panel.UserInfo) (option.Inbound, error) {
	...
	case "mieru":
		mieruUsers := make([]mieruinbound.User, len(users))
		for i := range users {
			mieruUsers[i] = mieruinbound.User{
				Name:     users[i].Uuid,
				Password: users[i].Uuid,
			}
		}
		in.Type = "mieru"
		in.Options = &mieruinbound.Options{
			ListenOptions:       listen,
			Users:               mieruUsers,
			Transport:           strings.ToUpper(info.Mieru.Transport),
			TrafficPattern:      info.Mieru.TrafficPattern,
			MTU:                 info.Mieru.MTU,
			UserHintIsMandatory: info.Mieru.UserHintIsMandatory,
		}
	case "hysteria":
		in.Type = "hysteria"
...
func (b *Sing) AddNode(tag string, info *panel.NodeInfo, config *conf.Options) error {
	...
	if info.Type == "mieru" {
		b.mieruLock.Lock()
		b.mieruNodes[tag] = &mieruNodeState{
			info:   info,
			config: config,
	...
	c, err := getInboundOptions(tag, info, config, nil)
	if err != nil {
		return err
	}
	in := b.box.Inbound()
...
func (b *Sing) DelNode(tag string) error {
	in := b.box.Inbound()
	b.mieruLock.Lock()
	_, isMieru := b.mieruNodes[tag]
	if isMieru {
		delete(b.mieruNodes, tag)
	}
	b.mieruLock.Unlock()
	if isMieru {
...
func (b *Sing) reloadMieruInbound(tag string, state *mieruNodeState) error {
	inboundManager := b.box.Inbound()
	if _, found := inboundManager.Get(tag); found {
		if err := inboundManager.Remove(tag); err != nil {
			return fmt.Errorf("delete mieru inbound error: %s", err)
		}
	}
	...
	inboundOptions, err := getInboundOptions(tag, state.info, state.config, users)
	if err != nil {
		return err
	}
	...
	if err != nil {
		return fmt.Errorf("add mieru inbound error: %s", err)
	}
	return nil
}
