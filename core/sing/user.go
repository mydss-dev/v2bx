...
func (b *Sing) AddUsers(p *core.AddUsersParams) (added int, err error) {
	if p.NodeInfo.Type == "mieru" {
		...
		b.mieruLock.Lock()
		defer b.mieruLock.Unlock()
		state, found := b.mieruNodes[p.Tag]
		if !found {
			return 0, errors.New("the mieru node not found")
		}
		for i := range p.Users {
			state.users[p.Users[i].Uuid] = p.Users[i]
			b.users.uidMap[p.Users[i].Uuid] = p.Users[i].Id
		}
		if err = b.reloadMieruInbound(p.Tag, state); err != nil {
			return 0, err
		}
...
func (b *Sing) DelUsers(users []panel.UserInfo, tag string, info *panel.NodeInfo) error {
	if info.Type == "mieru" {
		...
		b.mieruLock.Lock()
		defer b.mieruLock.Unlock()
		state, found := b.mieruNodes[tag]
		if !found {
			return errors.New("the mieru node not found")
		}
		for i := range users {
			if v, ok := b.hookServer.counter.Load(tag); ok {
				v.(*counter.TrafficCounter).Delete(users[i].Uuid)
			}
			delete(state.users, users[i].Uuid)
			delete(b.users.uidMap, users[i].Uuid)
		}
		return b.reloadMieruInbound(tag, state)
	}
	var del UserDeleter
