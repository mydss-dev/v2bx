...
type Sing struct {
	...
	mieruNodes                map[string]*mieruNodeState
	mieruLock                 sync.Mutex
}
...
func New(c *conf.CoreConfig) (vCore.Core, error) {
	...
	mieruinbound.RegisterInbound(inboundRegistry)
	ctx = box.Context(ctx, inboundRegistry, include.OutboundRegistry(), include.EndpointRegistry(), include.DNSTransportRegistry(), include.ServiceRegistry())
	...
	return &Sing{
		...
		mieruNodes:                make(map[string]*mieruNodeState),
	}, nil
}
